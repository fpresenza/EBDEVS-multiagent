...
from atomics.integrators.qsstools import advance_time, minposroot
...
class QSSIntegrator(AtomicDEVS):
    ...
    def __init__(self, name=None, dQMin=1e-6, dQRel=1e-3, gain=1, x0=0,
                 debug=False):
        """
        Constructor (parameterizable).
        """
        ...
        if self.debug:
            ...
            print("Init Function @ {} - t0: {}, q0: {}, x0: {}, sigma0: {}"
    ...
    def intTransition(self):
        ...
        current_time += sigma
        # TODO: replace by x = advance_time(xprev,sigma,1) # p: x, dt: sigma,
        # order: 1
        ...
        if self.debug:
            print("Internal Transition Function @ {} - t: {}, xprev: {}, "
                  "x: {}, q: {}, sigma: {}"
    ...
    def extTransition(self, inputs):
        ...
        derx_val = derx * self.gain  # [dx[i]*gain for i in range(len(dx))]
        ...
        if self.IN_dx in inputs:
            ...
            if (sigma > 0):
                ...
                if self.debug:
                    print("External Transition Function @ {} - t: {}, dx: {}, "
                          "x: {}, sigma: {}, sigma_lo: {}, sigma_up: {}"
                          .format(self.name, current_time, derx_val, x,
                                  sigma, sigma_lo, sigma_up)
                          )
    ...
    def outputFnc(self):
        ...
        # this change in q will be performed right after in the internal
        # transition function
        ...
        if self.debug:
            print("Output Function @ {} - t: {} xprev: {}, y: {}"
    ...
    def timeAdvance(self):
        ...
        if self.debug:
            print("Time Advance Function @ {} - t: {}, sigma: {}"
...
# -----------------------------------
# QSS_Integrator with Y_up: QSS integrator with micro-macro state
# communication with its parent.
# This class derives from QSSIntegrator => it's only necessary to
# reimplement the input and output transition functions.
# -----------------------------------
class mQSSIntegrator(QSSIntegrator):
    ...
    def intTransition(self):  # re-implement this function
        ...
        if (self.debug):
            print("t: {:.2f} s, Atomic name: {}, Internal Transition Function,"
                  " xprev: {}, x: {}, q: {}, sigma: {}"
                  .format(current_time, self.name, xprev, x, q, sigma)
                  )

        # shares information to the parent to compute the Global Transition
        # function
        try:
            self.y_up[2] = q.copy()
    ...
    def extTransition(self, inputs):
        ...
        if self.IN_dx in inputs:
            ...
            if (sigma > 0):
                ...
                if (self.debug):
                    print("t: {:.2f} s, Atomic name: {}, External Transition "
                          "Function, dx: {}, x: {}, sigma: {}, sigma_lo: {}, "
                          "sigma_up: {}"
                          .format(current_time, self.name, derx_val, x, sigma,
                                  sigma_lo, sigma_up)
                          )
