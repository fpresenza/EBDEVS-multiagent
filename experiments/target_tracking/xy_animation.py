...
print(
    'Average time between frames Q_1={:.5f} sec, \
        Q_2={:.5f} sec, Q_3={:.5f} sec'.format(
...
for target in targets_config.values():
    circle = plt.Circle(
        np.array(target['position'], dtype=float), target['collect_range'],
        color='r', alpha=0.3
    )
