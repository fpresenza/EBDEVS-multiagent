...
for i, robot in enumerate(robot_ids):
    ...
    kalman_data = np.loadtxt(
        experiment_directory + 'kalman_{}.csv'.format(robot), delimiter=','
    )
...
for i, robot in enumerate(robot_ids):
    control_data = np.loadtxt(
        experiment_directory + 'controller_{}.csv'.format(robot), delimiter=','
    )
