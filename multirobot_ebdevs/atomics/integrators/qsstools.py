...
def minposroot(coeff, order):
    ...
    # ORDER 1
        ...
        if (coeff[1] == 0):  # constant polynomial
            ...
            # if x(t) = coeff[0] + coeff[1] * t
            # => 0 = coeff[0] + coeff[1] * t0 => t0 = -coeff[0]/coeff[1]
            mpr = -coeff[0] / coeff[1]
    ...
    # ORDER 2
        ...
        if (coeff[2] == 0 or (1000 * abs(coeff[2])) < abs(coeff[1])):
            # linear polynomial
            if (coeff[1] == 0):  # constant polynomial
                mpr = INFINITY
    ...
    # ORDER 3
        ...
        if ((coeff[3] == 0) or (1000 * abs(coeff[3]) < abs(coeff[2]))):
            ...
            q = (
                3 * coeff[3] * coeff[1] - coeff[2] * coeff[2]
            ) / 9 / coeff[3] / coeff[3]
            r = (
                9 * coeff[3] * coeff[2] * coeff[1] -
                27 * coeff[3] * coeff[3] * coeff[0] -
                2 * coeff[2] * coeff[2] * coeff[2]
            ) / 54 / coeff[3] / coeff[3] / coeff[3]
    ...
    # ORDER 4
        ...
        if ((coeff[4] == 0) or (1000 * abs(coeff[4]) < abs(coeff[3]))):
            ...
            p = -3 * coeff[3] * coeff[3] / 8 / coeff[4] / coeff[4] + \
                coeff[2] / coeff[4]
            q = coeff[3] * coeff[3] * coeff[3] / \
                8 / coeff[4] / coeff[4] / coeff[4] - \
                coeff[3] * coeff[2] / 2 / coeff[4] / coeff[4] + \
                coeff[1] / coeff[4]
            r = -3 * np.power(coeff[3], 4) / 256 / np.power(coeff[4], 4) + \
                coeff[2] * coeff[3] * coeff[3] / 16 / np.power(coeff[4], 3) - \
                coeff[3] * coeff[1] / 4 / coeff[4] / coeff[4] + \
                coeff[0] / coeff[4]
            co = []
            co.append(-q * q)
            co.append(p * p - 4 * r)
    ...
    # Verification
    if(mpr >= 0):
        # some floting point calculations can return nan (or -nan),
        # so checking for "mpr < 0" might not be enough
        return mpr
    else:
        return INFINITY
...
#   else:
#       raise ValueError(
#           'Supplied QSS order (order={}) not implemented'.format(order)
#       )
...
def evaluate_poly(coeff, dt, order=1, debug=False):
    ...
    if (debug):
        print(
            "Evaluate Poly: coeff = {}, dt = {}, order = {}, eval = {}"
            .format(coeff, dt, order, p)
        )
    return float(p)
